...
# split into cells
grid = [re.split(r'\s', line) for line in re.split(r'\n', grid.strip())]
# parse integers
grid = map(lambda line : map(lambda element : int(element), line), grid)
...
print(max([prod for prod in iter_grid_products(grid, 4)]))
